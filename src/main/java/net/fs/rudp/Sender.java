...
public class Sender {
    ...
    void sendData(byte[] data, int offset, int length) throws InterruptedException {
    ...
    void sendNata(byte[] data, int offset, int length) throws InterruptedException {
        ...
        if (!closed) {
            if (!streamClosed) {
                ...
                ConsoleLogger.error("RDP连接已断开sendData");
                System.exit(-1);
            }
        } else {
            ConsoleLogger.error("RDP连接已经关闭");
            System.exit(-1);
        }
